...
const el = {
  ...
  checkoutSearch: document.getElementById('checkoutSearch'),
...
async function fetchSupabaseBootstrap() {
  ...
  const [
    ...
    supabaseClient.from('app_settings').select('*').order('created_at', { ascending: false }).limit(1),
    ...
    supabaseClient.from('shifts').select('*').order('created_at', { ascending: false }).limit(1),
  ]);
  ...
  if (hasError) {
    throw new Error(hasError.error.message || 'Supabase fetch failed.');
  }

  const orderItemsByOrder = new Map();
  (orderItemsRes.data || []).forEach((row) => {
    if (!orderItemsByOrder.has(row.order_id)) {
      orderItemsByOrder.set(row.order_id, []);
    }
    orderItemsByOrder.get(row.order_id).push({
      id: row.product_id || idGen(),
      name: row.item_name,
  ...
  return {
    settings: settingsRow
      ? {
          cafeName: settingsRow.cafe_name || state.settings.cafeName,
          address: settingsRow.address || state.settings.address,
          phone: settingsRow.phone || state.settings.phone,
          receiptQrImage: settingsRow.receipt_qr_image || '',
        }
      : null,
    ...
    customers: (customersRes.data || []).map((row) => ({
      ...
      phone: row.phone || '-',
      email: row.email || '',
      createdAt: row.created_at || nowStamp(),
    })),
    orders: (ordersRes.data || []).map((row) => ({
      id: row.order_code || row.id,
      dbId: row.id,
      time: row.created_at ? new Date(row.created_at).toLocaleString() : nowStamp(),
      customerName: row.customer_name || 'Walk-in Customer',
      items: orderItemsByOrder.get(row.id) || [],
      total: Number(row.total || 0),
      paymentMethod: row.payment_method || 'Pending',
      paymentStatus: row.payment_status || 'Unpaid',
      fulfillmentStatus: row.fulfillment_status || 'Preparing',
    ...
    transactions: (transactionsRes.data || []).map((row) => ({
      id: row.transaction_code || row.id,
      orderId: row.order_id || '',
      date: row.created_at ? new Date(row.created_at).toLocaleString() : nowStamp(),
    ...
    shift: shiftRow
      ? {
          isOpen: Boolean(shiftRow.is_open),
          openingAmount: Number(shiftRow.opening_amount || 0),
          drawerAmount: Number(shiftRow.drawer_amount || 0),
          changeReturned: Number(shiftRow.change_returned || 0),
        }
      : null,
  };
}
...
async function reloadFromSupabase() {
  try {
    const data = await fetchSupabaseBootstrap();
    if (!data) return false;
    apiEnabled = true;
    applyServerData(data);
    return true;
  } catch (error) {
    console.warn('Supabase connection failed. Using local state.', error);
    return false;
  }
}
...
function renderCheckoutProducts() {
  ...
  const query = String(el.checkoutSearch?.value || '').trim().toLowerCase();
  ...
  if (!filtered.length) {
    el.checkoutProducts.innerHTML = '<p>No matching products.</p>';
    return;
  }
...
function bindEvents() {
  ...
  el.checkoutSearch.addEventListener('input', renderCheckoutProducts);
